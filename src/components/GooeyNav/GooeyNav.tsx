...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  // Update active index when items change
  useEffect(() => {
    const newActiveIndex = items.findIndex(item => item.isActive);
    if (newActiveIndex !== -1 && newActiveIndex !== activeIndex) {
      setActiveIndex(newActiveIndex);
      const activeLi = navRef.current?.querySelectorAll("li")[newActiveIndex] as HTMLElement;
      if (activeLi) {
        updateEffectPosition(activeLi);
        textRef.current?.classList.add("active");
      }
    ...
    <>
      ...
      <style>
        {`
          ...
          .effect.filter {
            filter: blur(1px) contrast(200) blur(1px);
